...
def full_training_command(rank: int, row: pd.Series, datasets_dir: Path) -> list[str]:
    ...
    results_dir = (
        Path("gen_syntdata/experiments_gans_falls/full_data_full_results")
...
def write_top_results_report(output_dir: Path, datasets_dir: Path) -> None:
    summary_path = output_dir / "summary_mean_std.csv"
    if not summary_path.exists():
        return
    summary = pd.read_csv(summary_path)
    score_columns = [
        column for column in ["test_roc_auc_mean", "test_pr_auc_mean"] if column in summary.columns
    ]
    if summary.empty or not score_columns:
        (output_dir / "best_partitions_by_model.txt").write_text(
            "No complete smoke results available.\n", encoding="utf-8"
        )
        return
    ...
    top_ten = ranked.head(10)
    lines = [
        "Top 10 smoke configurations (global ranking)",
        "============================================",
        "",
    ...
    lines.extend(
        [
            "",
            "Full training commands for the top 3",
            "====================================",
            "",
            "Each command runs 10 seeds, 25 rounds and 100 local epochs.",
            "They use all 259942 real rows; GAN fitting uses training clients only.",
            "",
        ]
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the 60 falls-only GAN smoke tests.")
    parser.add_argument(
        "--datasets-dir",
        type=Path,
        default=Path("gen_syntdata/experiments_gans_falls/full_data_datasets"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("gen_syntdata/experiments_gans_falls/full_data_smoke_results"),
    )
    ...
    print(
        f"Grid: {len(GENERATOR_NAMES)} GANs x {len(RATIOS)} repartitions x "
        f"{len(STRATEGIES)} strategies x {len(args.seeds)} seed(s) = {total} runs"
    )

    command = [
        sys.executable,
        "-m",
        "gen_syntdata.smoke_experiments.run_fl_smoke_grid",
        "--datasets-dir",
    ...
    write_top_results_report(args.output_dir, args.datasets_dir)
