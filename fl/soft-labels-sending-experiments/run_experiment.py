...
from payload_configs import DEFAULT_PAYLOAD_ORDER, PAYLOAD_CONFIGS
from probability_payloads import ProbabilityPayloadStrategy, make_probability_payload_client_fn, require_flwr
...
def run_probability_payload_experiment(
    payload_name: str,
    results_dir: Path,
    ...
    if payload_name not in PAYLOAD_CONFIGS:
    ...
    client_fn = make_probability_payload_client_fn(setup, config, payload_name=payload_name, client_states=client_states)
    payload_strategy = ProbabilityPayloadStrategy(payload_name)

    class PayloadFedAvg(fl.server.strategy.FedAvg):
        ...
        def aggregate_fit(self, server_round, results, failures):
            ...
            self.latest_payload = payload_strategy.aggregate_payloads(results, weighted=bool(config.weighted_aggregation))
    ...
    config_payload["payload_description"] = PAYLOAD_CONFIGS[payload_name].description
    config_payload["communicates_model_weights"] = False
    config_payload["payload_scalar_count"] = PAYLOAD_CONFIGS[payload_name].scalar_count
    ...
    summary = {
        ...
        "payload_description": PAYLOAD_CONFIGS[payload_name].description,
        "payload_scalar_count": PAYLOAD_CONFIGS[payload_name].scalar_count,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run soft-label probability payload experiments.")
    parser.add_argument("--payload", choices=DEFAULT_PAYLOAD_ORDER, required=True)
    ...
    for run_idx in range(args.runs):
        ...
        run_dir = run_probability_payload_experiment(
