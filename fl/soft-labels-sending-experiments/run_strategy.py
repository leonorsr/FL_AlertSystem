...
from advanced_probability_strategies import AdvancedPayloadAggregator, make_advanced_probability_client_fn, require_flwr
from advanced_strategy_configs import ADVANCED_STRATEGIES, DEFAULT_ADVANCED_STRATEGY_ORDER
...
DEFAULT_RESULTS_ROOT = THIS_ROOT / "advanced_results"
...
def _collect_final_client_metrics(setup, config, client_states: dict) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for client_id, state in client_states.items():
        ...
        dev_rows.append({"client_id": client_id, **evaluate_model(model, setup.dev_arrays[0], setup.dev_arrays[1])})
        test_rows.append({"client_id": client_id, **evaluate_model(model, setup.test_arrays[0], setup.test_arrays[1])})
...
def _select_payload_for_client(strategy_payload: dict | None, cluster_id: int) -> dict | None:
    if not strategy_payload:
        return None
    if strategy_payload.get("scope") == "cluster":
        clusters = strategy_payload.get("clusters", {})
        return clusters.get(str(cluster_id))
    return strategy_payload.get("global")


def run_advanced_strategy_experiment(
    strategy_name: str,
    results_dir: Path,
    ...
    client_fn = make_advanced_probability_client_fn(setup, config, strategy_name=strategy_name, client_states=client_states)
    payload_aggregator = AdvancedPayloadAggregator(strategy_name)
    ...
    class AdvancedFedAvg(fl.server.strategy.FedAvg):
        ...
        def build_fit_config(self, server_round: int) -> dict:
            ...
            if server_round <= int(strategy_config.warmup_rounds):
                return fit_config
            if self.latest_payload is not None:
                fit_config["strategy_payload"] = json.dumps(self.latest_payload, ensure_ascii=True)
            return fit_config

        def aggregate_fit(self, server_round, results, failures):
            if failures:
                raise RuntimeError(f"Flower reported {len(failures)} failures in round {server_round}: {failures[0]!r}")
            self.latest_payload = payload_aggregator.aggregate_payloads(results, weighted=bool(config.weighted_aggregation))
        ...
        if source_params is not None:
            for partition in setup.fine_tune_clients.values():
                ...
                fine_tune_rows.append({
                    ...
                    **evaluate_model(local_model, partition.x_eval, partition.y_eval),
                })
    fine_tune_df = pd.DataFrame(fine_tune_rows)
    fine_tune_summary = _mean_summary(fine_tune_df)

    run_dir = ensure_directory(results_dir / strategy_name / f"run_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    config_payload = config_to_dict(config)
    config_payload["advanced_strategy_name"] = strategy_name
    config_payload["advanced_strategy_description"] = strategy_config.description
    config_payload["payload_name"] = strategy_config.payload_name
    config_payload["payload_scalar_count"] = payload_config.scalar_count
    config_payload["aggregation_scope"] = strategy_config.aggregation_scope
    config_payload["warmup_rounds"] = strategy_config.warmup_rounds
    config_payload["uncertain_only"] = strategy_config.uncertain_only
    ...
    history_payload = {
        ...
        "latest_strategy_payload": strategy.latest_payload,
    }
    ...
    summary = {
        ...
        "title": config.title,
        "advanced_strategy_name": strategy_name,
        "advanced_strategy_description": strategy_config.description,
        "payload_name": strategy_config.payload_name,
        "payload_scalar_count": payload_config.scalar_count,
        "aggregation_scope": strategy_config.aggregation_scope,
        "warmup_rounds": strategy_config.warmup_rounds,
        "uncertain_only": strategy_config.uncertain_only,
        ...
        "latest_strategy_payload": strategy.latest_payload or {},
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run advanced soft-label sending strategies.")
    parser.add_argument("--strategy", required=True, choices=DEFAULT_ADVANCED_STRATEGY_ORDER)
    ...
    for run_idx in range(args.runs):
        run_dir = run_advanced_strategy_experiment(
            ...
            random_seed_override=base_seed + run_idx,
        )
