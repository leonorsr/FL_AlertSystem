...
class FederatedSetup:
    ...
    fine_tune_clients: dict[str, ClientPartition] = field(default_factory=dict)
    client_summary: Any = None
...
def build_federated_setup(config: ExperimentConfig) -> FederatedSetup:
    ...
    fine_tune_clients: dict[str, ClientPartition] = {}
    for idx, (client_id, client_frame) in enumerate(test_frame.groupby("client_id", sort=True)):
        ...
        fine_tune_clients[client_id] = ClientPartition(
            client_id=client_id,
            dataset=dataset_name,
            client=client_name,
