...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate a summary for binned soft-label experiments.")
    parser.add_argument("--results-dir", default=str(DEFAULT_RESULTS_ROOT))
    parser.add_argument("--output", default="binned_probability_payload_summary.txt")
    args = parser.parse_args()

    results_dir = Path(args.results_dir)
    rows = []
    for payload_name in DEFAULT_BIN_PAYLOAD_ORDER:
    ...
    lines = ["Binned Soft-label Probability Payload Experiments", "================================================", ""]
    if df.empty:
        lines.append("No completed runs found.")
    else:
        summary_df = (
            df.groupby(["payload", "bins", "scalars"], as_index=False)
            .agg(
                runs=("run_dir", "count"),
        ...
        for _, row in summary_df.iterrows():
            lines.append(
                f"- {row['payload']} ({int(row['bins'])} bins, {int(row['scalars'])} scalars, runs={int(row['runs'])}): "
