...
from advanced_class_prototypes import AdvancedPrototypeStrategy, make_advanced_prototype_client_fn, require_flwr
...
from prototype_strategy_configs import DEFAULT_PROTOTYPE_STRATEGY_ORDER, PROTOTYPE_STRATEGIES


DEFAULT_RESULTS_ROOT = THIS_ROOT / "results"
DEFAULT_EXPERIMENT_ID = "exp14_final_clustered"
...
def run_prototype_strategy_experiment(
    strategy_name: str,
    results_dir: Path,
    ...
    if strategy_name not in PROTOTYPE_STRATEGIES:
    ...
    client_fn = make_advanced_prototype_client_fn(setup, config, strategy_name=strategy_name, client_states=client_states)
    prototype_strategy = AdvancedPrototypeStrategy(strategy_name)

    class PrototypeFedAvg(fl.server.strategy.FedAvg):
        ...
        def build_fit_config(self, server_round: int) -> dict:
            ...
            if self.latest_payload is not None:
                fit_config["global_prototype_payload"] = json.dumps(self.latest_payload, ensure_ascii=True)
            return fit_config

        def aggregate_fit(self, server_round, results, failures):
            if failures:
                raise RuntimeError(f"Flower reported {len(failures)} failures in round {server_round}: {failures[0]!r}")
            self.latest_payload = prototype_strategy.aggregate_payloads(results)
    ...
    config_payload["communication_mode"] = "advanced_class_prototype"
    config_payload["communicates_model_weights"] = False
    config_payload["payload_scalar_count"] = strategy_config.scalar_count
    config_payload["num_prototypes_per_class"] = strategy_config.num_prototypes_per_class
    config_payload["alignment_loss"] = strategy_config.alignment_loss
    ...
    history_payload = {
        ...
        "latest_prototype_payload": strategy.latest_payload,
    }
    ...
    summary = {
        ...
        "communication_mode": "advanced_class_prototype",
        "communicates_model_weights": False,
        "payload_scalar_count": strategy_config.scalar_count,
        "num_prototypes_per_class": strategy_config.num_prototypes_per_class,
        "alignment_loss": strategy_config.alignment_loss,
        ...
        "latest_prototype_payload": strategy.latest_payload or {},
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run one advanced class-prototype experiment.")
    parser.add_argument("--strategy", choices=DEFAULT_PROTOTYPE_STRATEGY_ORDER, required=True)
    ...
    for run_idx in range(args.runs):
        ...
        run_dir = run_prototype_strategy_experiment(
