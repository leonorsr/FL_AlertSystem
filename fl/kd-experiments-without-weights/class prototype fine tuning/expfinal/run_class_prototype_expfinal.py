...
BASE_EXPERIMENT_ID = "exp14_final_clustered"
...
def _write_report(run_dirs: list[Path], config) -> Path:
    ...
    lines = [
        "Class Prototype Final Local-First Experiment",
        "============================================",
        "",
        "Selection rationale",
        "-------------------",
        "This final experiment uses the best local-first configuration found during the",
        "lightweight class-prototype search: exp14_final_clustered with c2_lower_dropout.",
        "",
        "Configuration",
        "-------------",
        f"- base experiment: {BASE_EXPERIMENT_ID}",
        "- communication mode: class_prototype",
        ...
        f"- similarity groups: {config.num_similarity_groups}",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the selected local-first class-prototype final experiment.")
    parser.add_argument("--random-seed", type=int, default=5600)
    ...
    final_config = replace(
        base_config,
        model=replace(base_config.model, learning_rate=8e-4, dropout=0.20),
    ...
    print(f"Wrote final report: {_write_report(completed_runs, final_config)}")
