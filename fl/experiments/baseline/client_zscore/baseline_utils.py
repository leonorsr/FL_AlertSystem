...
DEFAULT_WINDOWS_PATH = REPO_ROOT / "data" / "preprocessing_results" / "simple" / "client_zscore" / "windows.csv"
...
def split_trials_by_dataset(trial_table: pd.DataFrame, config: SplitConfig) -> pd.DataFrame:
    ...
    for dataset_name, dataset_trials in trial_table.groupby("dataset", sort=True):
        ...
        stratify_first = _stratify_or_none(dataset_trials["label"])
        ...
        stratify_second = _stratify_or_none(temp_trials["label"])
        dev_trials, test_trials = train_test_split(
            temp_trials,
            test_size=0.5,
            random_state=config.random_state,
        ...
        train_trials = train_trials.assign(split="train")
        dev_trials = dev_trials.assign(split="dev")
        test_trials = test_trials.assign(split="test")
        split_frames.extend([train_trials, dev_trials, test_trials])

    split_df = pd.concat(split_frames, ignore_index=True)
    return split_df.sort_values(["dataset", "split", "trial_id"]).reset_index(drop=True)
...
def build_split_summary(split_df: pd.DataFrame) -> pd.DataFrame:
    summary = (
        ...
        .agg(trials=("trial_id", "nunique"), positive_trials=("label", "sum"), windows=("windows", "sum"))
