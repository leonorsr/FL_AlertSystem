...
from baseline_utils import (
    ...
    get_feature_columns,
    get_holdout_datasets,
    load_baseline_windows,
    split_trials_leave_one_dataset_out,
    write_json,
)
...
def build_scenarios(
    ...
    scenarios: list[dict] = []
    for holdout_dataset in get_holdout_datasets(trial_table):
        split_df = split_trials_leave_one_dataset_out(trial_table, holdout_dataset, split_config)
        labeled_df = attach_splits(windows_df, split_df)
        scenario_df = labeled_df[labeled_df["held_out_dataset"] == holdout_dataset].copy().reset_index(drop=True)
        scenarios.append(
            {
...
def select_best_classical_models(
    ...
    scenario_test_rows: list[dict] = []
    scenario_dataset_rows: list[dict] = []
    ...
    for family, candidates in grouped_candidates.items():
        ...
        for candidate_name, candidate_model in candidates:
            ...
            for scenario in scenarios:
                ...
                dev_selection_rows.append(
                    {
                        "held_out_dataset": scenario["held_out_dataset"],
                        "model_family": family,
                        "candidate": candidate_name,
                        **dev_metrics,
                    }
                )
                scenario_f1s.append(float(f1_score(y_dev, predictions["dev"]["predictions"], zero_division=0)))

            mean_f1 = float(np.mean(scenario_f1s)) if scenario_f1s else -1.0
        ...
        chosen_models.append(
            {
                ...
                "selection_metric": "mean_dev_f1_across_holdouts",
        ...
        for scenario in scenarios:
            ...
            scenario_test_rows.append(global_row)
            for row in dataset_rows:
                row["selected_candidate"] = best_candidate_name
                scenario_dataset_rows.append(row)
    ...
    return dev_selection_rows, scenario_test_rows, scenario_dataset_rows, chosen_models
...
def select_best_neural_model(
    ...
    if best_config is None:
        return dev_selection_rows, scenario_test_rows, scenario_dataset_rows, {
            "model_family": "neural_network",
    ...
    return dev_selection_rows, scenario_test_rows, scenario_dataset_rows, chosen_model


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run centralized leave-one-dataset-out experiments on baseline_zscore windows.")
...
def main() -> None:
    ...
    scenarios = build_scenarios(windows_df, trial_table, feature_columns, split_config)

    split_summary_frames = [build_split_summary(scenario["split_df"]) for scenario in scenarios]
    split_summary_df = pd.concat(split_summary_frames, ignore_index=True)
    split_summary_df.to_csv(run_dir / "split_summary.csv", index=False)

    selection_rows, scenario_global_rows, scenario_dataset_rows, chosen_rows = select_best_classical_models(
        scenarios=scenarios,
        seed=args.seed,
    )
    nn_selection_rows, nn_scenario_global_rows, nn_scenario_dataset_rows, nn_chosen_row = select_best_neural_model(
        scenarios=scenarios,
        seed=args.seed,
    )

    selection_rows.extend(nn_selection_rows)
    scenario_global_rows.extend(nn_scenario_global_rows)
    scenario_dataset_rows.extend(nn_scenario_dataset_rows)
    chosen_rows.append(nn_chosen_row)

    selection_df = pd.DataFrame(selection_rows).sort_values(["model_family", "held_out_dataset", "candidate"]).reset_index(drop=True)
    scenario_global_df = pd.DataFrame(scenario_global_rows).sort_values(["held_out_dataset", *FINAL_SORT_COLUMNS], ascending=[True, *FINAL_SORT_ASCENDING]).reset_index(drop=True)
    scenario_dataset_df = pd.DataFrame(scenario_dataset_rows).sort_values(["held_out_dataset", "dataset", *FINAL_SORT_COLUMNS], ascending=[True, True, *FINAL_SORT_ASCENDING]).reset_index(drop=True)
    chosen_models_df = pd.DataFrame(chosen_rows).sort_values("model_family").reset_index(drop=True)

    final_global_df = aggregate_metrics(scenario_global_rows, ["model", "split", "dataset"]).sort_values(FINAL_SORT_COLUMNS, ascending=FINAL_SORT_ASCENDING).reset_index(drop=True)
    final_dataset_df = aggregate_metrics(scenario_dataset_rows, ["model", "split", "dataset"]).sort_values(["dataset", *FINAL_SORT_COLUMNS], ascending=[True, *FINAL_SORT_ASCENDING]).reset_index(drop=True)

    selection_df.to_csv(run_dir / "model_selection_dev.csv", index=False)
    scenario_global_df.to_csv(run_dir / "scenario_model_metrics.csv", index=False)
    scenario_dataset_df.to_csv(run_dir / "scenario_model_metrics_by_dataset.csv", index=False)
    ...
    config_payload = {
        "seed": args.seed,
        "windows_path": str((Path(__file__).resolve().parents[3] / "data" / "preprocessing_results" / "simple" / "baseline_zscore" / "windows.csv").resolve()),
        "feature_count": len(feature_columns),
        "feature_columns": feature_columns,
        "split_config": dataclass_to_dict(split_config),
        "split_policy": "leave_one_dataset_out",
        "held_out_datasets": get_holdout_datasets(trial_table),
    ...
    print("")
    print("Final leave-one-dataset-out comparison:")
