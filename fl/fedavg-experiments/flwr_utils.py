...
from modeling import create_model, evaluate_model, get_model_parameters, predict_probabilities, set_model_parameters, train_local_model


@dataclass
class ClientState:
    best_global_params: list[np.ndarray] | None = None
    best_global_f1: float = -1.0
    current_local_params: list[np.ndarray] | None = None
    last_val_metrics: dict[str, float | int] | None = None
...
def require_flwr():
    try:
        ...
        raise ImportError(
            "Flower is not installed in the current environment. Install it with '..\\venv\\Scripts\\python.exe -m pip install flwr'."
        ) from exc
...
def make_client_fn(setup: FederatedSetup, config: ExperimentConfig):
    fl, _, _ = require_flwr()
    client_states: dict[str, ClientState] = {client_id: ClientState() for client_id in setup.train_clients}
    ordered_client_ids = sorted(setup.train_clients.keys())

    class FallNumPyClient(fl.client.NumPyClient):
        def __init__(self, partition: ClientPartition, state: ClientState) -> None:
        ...
        def fit(self, parameters, config_dict):
            ...
            if config.local_model_selection:
                ...
                if self.state.best_global_params is not None:
                    ...
                    previous_global_f1 = float(
                        evaluate_model(candidate_previous_global, self.partition.x_val, self.partition.y_val)["f1"]
                    )
    ...
    def client_fn(context):
        ...
        return FallNumPyClient(partition, client_states[client_id]).to_client()
...
def build_final_local_metrics_table(setup: FederatedSetup, config: ExperimentConfig, client_states: dict[str, ClientState]):
    rows: list[dict] = []
    for client_id, partition in setup.train_clients.items():
        state = client_states.get(client_id)
        params = None
        source = "current_local_model"
        if state is not None and state.current_local_params is not None:
            params = state.current_local_params
        elif state is not None and state.best_global_params is not None:
            params = state.best_global_params
